    ...
    >
      ...
      {selectedMember && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-5xl mx-4 rounded-xl shadow-lg overflow-hidden relative flex flex-col md:flex-row">
            {/* Left section */}
            <div className="w-full md:w-1/3 bg-gray-50 p-6 flex flex-col items-center justify-center relative">
              <button
                className="cursor-pointer absolute top-4 left-4 text-gray-500 hover:text-black"
                onClick={() => setSelectedMember(null)}
              >
              ...
              <div className="mt-6 text-center text-sm text-gray-600">
                {optOut ? (
                  <p className="text-red-500 italic">Contact info hidden (Opted out)</p>
                ) : (
                  <>
                    <p>{selectedMember.email}</p>
                    <p>{selectedMember.phone}</p>
              ...
              <label className="mt-4 flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                <input
                  type="checkbox"
                  checked={optOut}
                  onChange={() => setOptOut((prev) => !prev)}
                />
                Opt-out of contact visibility
              </label>
